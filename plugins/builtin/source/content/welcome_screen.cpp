...
namespace hex::plugin::builtin
{
    ...
    namespace
    {
        ...
        void drawTiles(ImDrawList *drawList, ImVec2 position, ImVec2 size, float lineDistance)
        {
                ...
                if (overCounter == 0)
                {
                    for (const auto &segment : segments)
                    {
                        ...
                        if (segment.x < 0 || segment.y < 0)
                            overCounter = 5;
                        if (segment.x > i32(tileCount.x) || segment.y > i32(tileCount.y))
                            overCounter = 5;
                    }
            ...
            if (ImGui::IsKeyDown(ImGuiKey_UpArrow) && direction != ImGuiDir_Down)
                direction = ImGuiDir_Up;
            if (ImGui::IsKeyDown(ImGuiKey_DownArrow) && direction != ImGuiDir_Up)
                direction = ImGuiDir_Down;
            if (ImGui::IsKeyDown(ImGuiKey_LeftArrow) && direction != ImGuiDir_Right)
                direction = ImGuiDir_Left;
            if (ImGui::IsKeyDown(ImGuiKey_RightArrow) && direction != ImGuiDir_Left)
                direction = ImGuiDir_Right;
        }
        ...
        void drawWelcomeScreenContentFull()
        {
            ...
            if (ImGui::BeginTable("Welcome Outer", 1, ImGuiTableFlags_None, ImGui::GetContentRegionAvail() - margin))
            {
                ...
                if (ImGui::BeginTable("Welcome Left", 1, ImGuiTableFlags_NoBordersInBody, ImVec2(availableSpace.x / 2, 0)))
                {
                    ...
                    {
                        if (ImGuiExt::IconHyperlink(ICON_VS_GITHUB, "hex.builtin.welcome.help.repo"_lang))
                            hex::openWebpage("hex.builtin.welcome.help.repo.link"_lang);
                        if (ImGuiExt::IconHyperlink(ICON_VS_ORGANIZATION, "hex.builtin.welcome.help.gethelp"_lang))
                            hex::openWebpage("hex.builtin.welcome.help.gethelp.link"_lang);
                        if (ImGuiExt::IconHyperlink(ICON_VS_COMMENT_DISCUSSION, "hex.builtin.welcome.help.discord"_lang))
                            hex::openWebpage("hex.builtin.welcome.help.discord.link"_lang);
                    }
                    ImGuiExt::EndSubWindow();
     ...
     *  函数在启动时调用，用于初始化欢迎屏幕的相关功能。它注册事件处理程序、更新最近条目、处理主题和语言设置的更改、检查崩溃备份文件、显示每  *  日提示，并根据环境条件显示捐赠横幅或加载信息横幅纹理。
     */
